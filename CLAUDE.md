...
- **Course notebook:** `make course` (the notebook as a marimo app over the real pipeline, no code shown; `make course-edit` for the editor; the course itself is at https://flashy-fur-afc.notion.site/quarterly-RAG-a-course-on-production-RAG-3d21f11d4bc881a6b753c2c819817428); `uv run python notebooks/course.py` runs the ungated cells as a script
