...
- **LLM and embeddings served by Ollama** (OpenAI-compatible local server). Concrete models are chosen in RAG-002 and recorded in ADR-005.
...
- **Two vector stores** (ChromaDB, FAISS) are implemented and benchmarked before a default is chosen (ADR-006, RAG-007).
