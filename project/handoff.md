...
**The backlog is ordered in `project/tickets.md` under "Phase 4" (RAG-038, 2026-09-05)**, and the next four are: RAG-039, which nine of the 33 answerable questions sit below rank 20 and why, a deterministic analysis with only the embedder involved; RAG-040, OpenAI's `gpt-5.6-sol` on the same 63 questions at an equal answer budget, with dollars and seconds per question, which Kevin asked for; RAG-041, the second labelling round, which Kevin reviews and which produces the paraphrase pairs RAG-032 needs; then RAG-032 itself, retrieval unstable to phrasing and only for Nvidia, with an ADR if a model goes into the retrieval path. After those: the gate writes per-question results (RAG-042) and covers `derived` and `cross_period` (RAG-043), the `q052` leak (RAG-044), the embedding comparison (RAG-045), a prompt v2 wording (RAG-046), the Langfuse environment mismatch (RAG-047) and a third company (RAG-048).
...
- **The course**: twelve Notion chapters, public at https://flashy-fur-afc.notion.site/quarterly-RAG-a-course-on-production-RAG-3d21f11d4bc881a6b753c2c819817428, and `notebooks/course.py`, opened with `make course`. Every chapter names its notebook section and every section links back to the public pages.
...
- No hosted model has been measured (RAG-040), and the embedding model comparison was never run (RAG-045).
