...
Phases one and two are complete except RAG-021. The pipeline answers from the filings or refuses with a reason, and every layer is measured against a 63-question human-verified eval set (33 answerable, 30 that must be refused). Ten ADRs under `docs/adr/` record the decisions; each has a tradeoff page with numbers under `docs/tradeoffs/`. The headline numbers are in the README under "Results so far" and in `data/eval/baseline.json`, which `make eval` gates against.

The next ticket is **RAG-021, calculation provenance for derived numbers**. Then phase three: RAG-013 Langfuse tracing, RAG-014 FastAPI and Streamlit, RAG-015 the results writeup. The backlog in `project/tickets.md` has the full text of each.
...
- `make test` runs 331 unit tests with no model. `make test-all` adds the live tests. `make eval` runs the regression gate and takes about five minutes.
...
Start by reading `CLAUDE.md`, running `make test` to confirm the tree is healthy, and claiming RAG-021.
