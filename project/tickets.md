...
- **Verified:** `scripts/edit_docs.py` reported every edit applied: tickets 4/4, handoff 4/4, `CLAUDE.md` 1/1, README 4/4. The three places that name the next ticket (`CLAUDE.md`, `project/handoff.md`, this file) all say RAG-039, and every open thread in the handoff and in the README's open list carries a ticket id. The model id `gpt-5.6-sol`, the $4 and $20 prices, the reasoning-effort levels and the context window were read from OpenAI's model page on 2026-09-05; the promotional window and the standard price come from third-party listings the same day and the ticket says so. A grep of the diff for the configured server host found nothing. `make lint` clean. No code changed, so the tests were not re-run.
