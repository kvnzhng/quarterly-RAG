...
- **Description:** RAG-021 left three cases where a correct answer fails the operand check, each seen in its measurement runs. (1) A unitless operand does not match a percentage: `qwen3.8-27b-64k` wrote `CALC: 15.6 [c1] - 24.1 [c1]` against a passage that prints `15.6%` and `24.1%`, and the check refused it because a plain number is not a percentage. (2) A calculation cannot consume another calculation's result: `llama3.1:8b` wrote `CALC: 8.5% [c1] / 24.1% [c1]` where the 8.5% came from its own earlier line, so it cites a passage for a figure no passage prints. (3) A scale constant written into a prose sentence is flagged as a figure the passage does not state: "we divide the gross margin by total net sales and multiply by 100" costs the answer its `figures accounted` mark for the 100 alone. Decide for each whether to widen the check or to tighten the prompt, and say which in the ticket.
